...
export default new Vuex.Store({
    ...
    actions: {
        ...
        async getScoreList({commit}) {
            const result = await reqScoreList()
            // 判断返回的code
            
            commit('GETSCORELIST', {scoreList: result.scoreList})
        },

        // 获取学生详细成绩
        async getScoreDetail({commit}, stuId) {
            const result = await reqScoreDetail(stuId)
            commit('REQSCOREDETAIL', {scoreDetail: result.scoreDetail})
        }
    },
    getters: {
